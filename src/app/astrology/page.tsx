...
import { ArrowLeft, Sparkles, Star, Heart, Briefcase, Moon, Sun, ChevronRight } from "lucide-react";
import {
    getZodiacSign, getMoonPhase, getDailyMessage, getCurrentPlanets,
    getElementAnalysis, ZODIAC_SIGNS, type ZodiacSign
} from "@/lib/astrology";
...
export default function AstrologyPage() {
    ...
    const handleDateSubmit = (e: React.FormEvent) => {
        ...
        setSelectedSign(getZodiacSign(m, d));
    };
        ...
        <div className="min-h-screen bg-[#0a0812] text-white relative overflow-hidden">
            ...
            <main className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 py-8 pb-20">
                ...
                <div className="text-center mb-10">
                    <h1 className="text-3xl sm:text-4xl font-heading font-semibold text-white/90 mb-2">
                        Astroloji
                    </h1>
                    <p className="text-sm text-white/35 max-w-sm mx-auto">
                        Doğum tarihinizi girin, yıldız haritanızı keşfedin.
                    </p>
                </div>
                ...
                {!selectedSign ? (
                    ...
                    >
                        <h2 className="text-sm font-semibold text-white/60 mb-4 flex items-center gap-2">
                            <Star className="w-4 h-4 text-purple-400/50" />
                            Doğum Tarihiniz
                        </h2>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1 mb-6 bg-white/[0.02] rounded-xl p-1 border border-white/[0.05]">
                                {([
                                    ...
                                    { key: "daily", label: "Günlük", icon: <Sun className="w-3.5 h-3.5" /> },
                                    { key: "planets", label: "Gezegenler", icon: <Moon className="w-3.5 h-3.5" /> },
                                ] as const).map(tab => (
                                    <button
                                        key={tab.key}
                                        onClick={() => setActiveTab(tab.key)}
                                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-semibold transition-all ${activeTab === tab.key
                                ...
                                {activeTab === "daily" && (
                                    <motion.div key="daily" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                                        {[
                                            ...
                                            <div key={item.cat} className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5">
                                                <div className="flex items-center gap-2 mb-3">
                                                    {item.icon}
                                                    <h3 className="text-xs font-semibold text-white/50">{item.label}</h3>
                                                </div>
                                                <p className="text-sm text-white/60 leading-relaxed italic">
                                                    &ldquo;{getDailyMessage(selectedSign.name, item.cat)}&rdquo;
                                                </p>
                                            </div>
                                        ))}
                                            ...
                                            <div key={i} className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-4 flex items-center gap-4">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <span className={`text-[10px] px-2 py-0.5 rounded-full ${p.status.includes("Retrograd") ? "bg-red-500/10 text-red-400/70 border border-red-500/20" : "bg-white/[0.04] text-white/30"}`}>
                                                            {p.status}
                                                        </span>
                                                    </div>
                            ...
                            {/* Change sign button */}
                            <button
                                onClick={() => { setSelectedSign(null); setActiveTab("overview"); }}
                ...
                {/* Zodiac Wheel - Quick Access (when sign is selected) */}
                {selectedSign && (
                    <div className="mt-6 flex items-center justify-center flex-wrap gap-1">
                        {ZODIAC_SIGNS.map(sign => (
                            <button
                                key={sign.id}
                                onClick={() => { setSelectedSign(sign); setActiveTab("overview"); }}
                                className={`w-9 h-9 rounded-full flex items-center justify-center text-lg transition-all ${selectedSign.id === sign.id
                                    ? "bg-purple-500/15 border border-purple-500/25 scale-110"
                                    : "hover:bg-white/[0.04] opacity-40 hover:opacity-70"
                                    }`}
                                title={sign.name}
                            >
                                {sign.symbol}
